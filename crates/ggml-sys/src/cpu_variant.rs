...
//! Nothing else in ggml or in onnxruntime-ggml names a CPU backend symbol directly: the rest goes
...
///
/// `ORT_GGML_CPU_VARIANT=baseline` (or `avx2`) overrides the detection. A machine can
...
fn haswell() -> bool {
    match std::env::var("ORT_GGML_CPU_VARIANT").ok().as_deref() {
