...
pub fn device_capable(op: &str) -> bool {
    DEVICE_OPS.contains(&op) || crate::exec::ops_extra::DEVICE_OPS.contains(&op)
}
...
pub struct Run<'p> {
    ...
    sequences: HashMap<String, std::sync::Arc<Vec<std::sync::Arc<HostTensor>>>>,
    uploads: Vec<Upload>,
...
impl<'p> Run<'p> {
    ...
    pub fn execute(&mut self, inputs: Vec<InputRef>) -> Result<Vec<HostTensor>> {
        ...
        for i in 0..n {
            ...
            if crate::host::eval_control::OPS.contains(&node.op.as_str())
                || (node.op == "Identity" && node.input(0).is_some_and(|name| self.sequences.contains_key(name)))
            {
                self.run_control(node)?;
                continue;
            }
            ...
            let outs = self.run_node(node, ins).map_err(|e| Error::model(format!("{node}: {e}")))?;
    ...
    fn run_node(&mut self, node: &Node, mut ins: Vec<Option<In>>) -> Result<Vec<Value>> {
        if crate::host::eval_random::OPS.contains(&node.op.as_str()) {
            // Like operators consume only input metadata, never activation data.
            let i = ins.first().and_then(|v| v.as_ref()).ok_or_else(|| Error::model("random input missing"))?;
            let shape = HostTensor::zeros(i.v.dtype(), i.v.shape());
            let outs = self.prog.random.lock().map_err(|_| Error::internal("random stream lock"))?.eval(node, &[Some(&shape)])?;
            self.stats.host_ops += 1;
            return Ok(outs.into_iter().map(Value::staged_of).collect());
        }
    ...
    fn forced_host(&self, node: &Node, ins: &[Option<In>]) -> bool {
        ...
        if crate::exec::ops_extra::DEVICE_OPS.contains(&op)
            && ins.first().and_then(|i| i.as_ref()).is_some_and(|i| !i.v.dtype().is_float())
        {
            return true;
        }
    ...
    fn emit(&mut self, node: &Node, ins: &[Option<In>]) -> Result<Vec<Value>> {
        match node.op.as_str() {
            "LSTM" => crate::exec::ops_lstm::emit(self, node, ins),
            op if crate::exec::ops_extra::DEVICE_OPS.contains(&op) => crate::exec::ops_extra::emit(self, node, ins),
